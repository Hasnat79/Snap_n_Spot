...
from data import charades
from post_process_outputs.utils import LightweightTimeExtractor
import argparse
import torch
...
if __name__ == "__main__":
    ...
    output_dir = "/scratch/user/hasnat.md.abdullah/Snap_n_Spot/outputs/vtime_llm_x_charades"
    os.makedirs(output_dir, exist_ok=True)

    if os.path.exists(f"{output_dir}/{args.output}"):
      with open(f"{output_dir}/{args.output}", "r") as f:
        results = json.load(f)
    else:
      results = {}
    processed = 0
    ...
    for i, (k, v) in tqdm(enumerate(charades.items())):
       print(i, k, v)
       video_path = os.path.join(video_dir, k + ".mp4")
       print(f"Video_path: {video_path}")
       features = process_video(video_path)
       v['vtime_llm_generation'] = []
       v['preds']= []
       for s in v['sentences']:
            ...
            print(f"prompt: {s}")
            try: 
                if k in results and len(results[k]['vtime_llm_generation'] == len(v['sentences'])):
                    print("Already processed")
                    continue
                vtime_llm_generation = inference(model, features, query, tokenizer)
                v['vtime_llm_generation'].append(vtime_llm_generation)
                result = extractor.extract_vtime_times(vtime_llm_generation)
                start,end = result['start'], result['end']
                v['preds'].append([start, end])

            except Exception as e:
                print(f"Error: {e}")
                v['vtime_llm_generation'].append("Error")
                v['preds'].append([None, None])
                with open(f"{output_dir}/{args.output}", "w") as f:
